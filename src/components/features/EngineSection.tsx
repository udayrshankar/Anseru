...
const EngineCard = ({ title, description, badge, index }: { title: string, description: string, badge: string, index: number }) => {
    return (
        <motion.div
            ...
            className="group relative rounded-[32px] bg-white p-2 border border-black/[0.03] shadow-sm hover:shadow-md transition-all"
        >
             <div className="flex flex-col lg:flex-row gap-8 bg-[#F6F6F8] rounded-[28px] p-8 min-h-[300px]">
                {/* Visual Placeholder */}
                <div className="flex-1 w-full min-h-[200px] bg-gradient-to-br from-white to-[#F0E6F5] rounded-2xl border border-white/50 shadow-inner flex items-center justify-center relative overflow-hidden">
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(192,132,252,0.1),transparent)]" />
                     ...
                     <div className="w-32 h-32 rounded-full border-4 border-[#C084FC]/10 flex items-center justify-center">
                        <div className="w-20 h-20 rounded-full bg-[#C084FC]/10" />
                     </div>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center">
                    <div className="mb-4">
                        <span className="inline-block px-3 py-1 rounded-full bg-white border border-black/[0.05] text-xs font-medium text-[#C084FC]">
                            {badge}
                        </span>
                    </div>
                    <h3 className="text-[#2A1638] text-2xl font-medium mb-4 leading-tight">
                        {title}
                    </h3>
                    <p className="text-[#483953]/70 text-lg leading-relaxed">
                        {description}
                    </p>
                </div>
             </div>
