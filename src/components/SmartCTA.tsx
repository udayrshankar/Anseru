...
const SmartCTA = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 bg-white border border-[#1C32E6]/30 rounded-[15px] px-6 h-[42px] shadow-[0_0_15px_rgba(28,50,230,0.15)] min-w-[180px] w-full sm:w-auto overflow-hidden relative">
         <span className="text-xl">✨</span>
         <div className="flex items-center text-[#393939] font-medium text-[15px] font-onest">
            <span className="relative">
                {displayedText.indexOf(' ') === -1 ? (
                    <span className="text-[#1C32E6] font-semibold">{displayedText}</span>
                ) : (
                    <>
                        <span className="text-[#1C32E6] font-semibold">{displayedText.slice(0, displayedText.indexOf(' '))}</span>
                        <span>{displayedText.slice(displayedText.indexOf(' '))}</span>
                    </>
                )}
                {/* Cursor */}
                <span className="absolute -right-[2px] top-0 h-full w-[2px] bg-[#1C32E6] animate-blink" />
            </span>
         </div>
      </div>
      ...
      {/* Book Demo Button */}
      <motion.button 
        className="px-6 h-[42px] rounded-full bg-black hover:bg-[#3a3a3a] text-white font-onest text-[15px] font-medium shadow-md hover:shadow-lg transition-all flex items-center gap-2 group relative overflow-hidden w-full sm:w-auto justify-center"
